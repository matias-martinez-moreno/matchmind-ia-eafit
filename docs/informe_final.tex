...
\begin{center}
  ...
  {\small\color{eafitgray}
    ...
    \textit{Entrega: miercoles 20 de mayo de 2026}\\[4pt]
    \textbf{Repositorio:}
...
\begin{abstract}
\noindent
\textbf{Contexto.} La prediccion de resultados de partidos de futbol es un problema clasico de clasificacion multiclase que combina alta varianza estocastica (los resultados son inciertos por naturaleza) con señales sistemicas predecibles (ranking, ventaja de localia, historial entre rivales). Es un caso de estudio ideal porque tiene datos abundantes, etiquetas claras y un techo teorico bajo: incluso los mejores modelos publicados rara vez superan accuracy de 0.60.
\textbf{Objetivo.} Construir un sistema completo que: (1) prediga el resultado de un partido internacional entre tres clases (victoria local, empate, victoria visitante), (2) explique la prediccion con valores SHAP, y (3) genere un analisis tactico narrativo en español mediante un LLM.
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{En español:} heuristica de palabras frecuentes en español.
...
La Figura~\ref{fig:cm} revela una limitacion importante: el modelo \textbf{subpredice los empates}. De 1,360 empates reales, solo predice 578 (el 42\%). El error mas frecuente es ``real = empate, predicho = victoria local'' (837 casos, 31.6\% de todos los errores). Los empates son intrinsecamente dificiles de predecir en futbol porque ocurren cuando dos equipos llegan justo a la misma cantidad de goles, lo cual es una condicion ``de frontera'' poco frecuente y altamente sensible a la estocasticidad del partido.
...
La Figura~\ref{fig:shap} confirma que la \textbf{diferencia de ranking FIFA es la feature dominante} (impacto promedio 0.385), seguida del head-to-head historico del visitante (0.190) y la diferencia de puntos FIFA (0.177). Una observacion contraintuitiva: la racha reciente (5 partidos) resulta menos importante de lo esperado. Probablemente esto se debe a que el ranking FIFA ya incorpora indirectamente el desempeño reciente, asi que las dos features se vuelven parcialmente redundantes.
...
Respuesta en español                   & \textbf{100\%} \\
...
El resultado es consistente con la dificultad intrinseca del problema. Predecir el resultado de un partido de futbol tiene un techo teorico bajo porque una parte importante del resultado es genuinamente aleatoria: rebotes en el poste, decisiones arbitrales, lesiones puntuales en el minuto 30. Estas fuentes de varianza no estan en los datos y limitan el F1 alcanzable independientemente del algoritmo elegido.
...
%% ==========================================================
%% REFERENCIAS (4 referencias, todas citadas en el texto)
%% ==========================================================
...
genera un pronostico narrativo en español
(MAXIMO 150 palabras).
...
5. Tono: profesional, conciso, español.
\end{lstlisting}
